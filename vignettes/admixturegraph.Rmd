...
The Bayes factor comparing the two models can be obtained just from subtracting one log-likelihood from the other, but we can obtain the Bayes factor together with the variation due to the numerical algorithm using the `model_bayes_factor_n` function:

```{r, cache = TRUE}
model_bayes_factor_n(thinned_1[,"likelihood"], thinned_2[,"likelihood"], 100)
```

The result here shows a very strong support for the first model compared to the second. While both models give a similar fit to the data, the second has many more parameters, and when integrating over the parameter space the more complex model is penalised for having these extra parameters. 
