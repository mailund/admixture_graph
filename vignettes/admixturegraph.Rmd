...
To add a new admixture event to an existing graph use either `add_an_admixture` or `add_an_admixture2`; the former only adding one parental edge assuming the other one needs to already exist in the graph and the latter detaching one edge and reattaching it with two parental edges anywhere in the original graph (provided that the result still is a valid admixture graph):
